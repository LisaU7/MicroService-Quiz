...
public class TriviaQuestionBuilderTest {
	...
	public void testBuild() {
		...
		TriviaQuestion result = instance.id(1).question("What was the first toy advertised on television?")
				.answerA("The Rubix Cube").answerB("Mr. Potato Head").answerC("Barbie").answerD("A Hula hoop")
				.correctAnswer("B").hint("Use your head on this one").lastUpdated(currentDate).build();
		assertNotNull("Trivia question build failed.", result);
		assertEquals("Ids do not match after build.", 1, result.getId());
		assertEquals("Questions do not match after build.", "What was the first toy advertised on television?",
				result.getQuestion());
	...
	public void testBuildIdFail() {
		(new TriviaQuestionBuilder()).id(-1).build();
	}
	...
	public void testBuildAnswerAFail() {
		(new TriviaQuestionBuilder()).answerA(null).build();
	}

	@Test(expected = IllegalArgumentException.class)
	public void testBuildAnswerBFail() {
		(new TriviaQuestionBuilder()).answerB(null).build();
	}

	@Test(expected = IllegalArgumentException.class)
	public void testBuildAnswerCFail() {
		(new TriviaQuestionBuilder()).answerC(null).build();
	}

	@Test(expected = IllegalArgumentException.class)
	public void testBuildAnswerDFail() {
		(new TriviaQuestionBuilder()).answerD(null).build();
	}

	@Test(expected = IllegalArgumentException.class)
	public void testBuildHintFail() {
		(new TriviaQuestionBuilder()).hint(null).build();
	}

	@Test(expected = IllegalArgumentException.class)
	public void testBuildCorrectAnswerFail() {
		(new TriviaQuestionBuilder()).correctAnswer(null).build();
	}

	@Test(expected = IllegalArgumentException.class)
	public void testBuildLastUpdatedDateFail() {
		(new TriviaQuestionBuilder()).lastUpdated(null).build();
	}
